...
//	echo 1;die;
        ...
        return $this->render('public_account_list',$str);//跳转到展示页�?
    }
    ...
    public function actionAdd()
    {
        ...
        if($method !="POST"){
            ...
            $pat_api_link='http://'.$_SERVER['HTTP_HOST']."/haoyunyun/web/wx_sample.php";//获取当前的URL不带参数
            ...
            if($str){
                ...
                $pat_token = '';
                for ($i=0; $i < 32; $i++)//随机生成一�?2位的token
                {
                    $pat_token.= $string[rand(0,strlen($string)-1)];
                }
                $pat_hash="";
                for ($i=0; $i < 5; $i++)//随机生成一�?位的hash
                {
